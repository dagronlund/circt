...
struct ModuleVisitor : public BaseVisitor {
  ...
  /// Make hierarchical outputs available before lowering any port connections.
  /// Temporary values allow forward and cyclic references between instances;
  /// they are replaced with the actual instance results when it is emitted.
  LogicalResult
  predeclareHierarchicalOutputs(const slang::ast::InstanceSymbol &instNode) {
    auto *body = getCanonicalBody(instNode);
    auto *moduleLowering = context.convertModuleHeader(body);
    if (!moduleLowering)
      return failure();
    auto types = moduleLowering->op.getModuleType().getOutputTypes();
    SmallVector<Value> outputs(types.size());
    for (const auto &path : context.hierPaths[body])
      if (path.idx && path.direction == slang::ast::ArgumentDirection::Out)
        outputs[*path.idx] = mlir::UnrealizedConversionCastOp::create(
                                 builder, loc, types[*path.idx], ValueRange{})
                                 .getResult(0);
    recordHierarchicalOutputs(instNode, outputs);
    return success();
  }

  void recordHierarchicalOutputs(const slang::ast::InstanceSymbol &instNode,
                                 ValueRange outputs) {
    ...
    for (const auto &hierPath : context.hierPaths[body])
      if (hierPath.idx && hierPath.direction == ArgumentDirection::Out) {
        auto result = outputs[*hierPath.idx];
        auto key = std::make_pair(&instNode, hierPath.hierName);
        if (auto previous = context.hierValueSymbols.lookup(key)) {
          if (auto placeholder =
                  previous.getDefiningOp<mlir::UnrealizedConversionCastOp>()) {
            previous.replaceAllUsesWith(result);
            placeholder.erase();
          }
        }
        ...
        context.hierValueSymbols[key] = result;
      }
  }
  ...
  LogicalResult visit(const slang::ast::InstanceSymbol &instNode) {
    ...
    recordHierarchicalOutputs(instNode, inst.getOutputs());
...
struct ModulePredeclaration {
  Context &context;
  OpBuilder &builder;
  bool declaringHierarchicalOutputs = false;
  ...
  LogicalResult predeclareModuleInstanceMember(const slang::ast::Symbol &member,
    ...
    if (const auto *instNode = member.as_if<slang::ast::InstanceSymbol>()) {
      if (instNode->body.getDefinition().definitionKind !=
          slang::ast::DefinitionKind::Interface) {
        if (declaringHierarchicalOutputs)
          return ModuleVisitor(context, loc, blockNamePrefix)
              .predeclareHierarchicalOutputs(*instNode);
        if (failed(
  ...
  LogicalResult predeclareScope(const slang::ast::Scope &scope,
    ...
    // Register hierarchical outputs across the whole scope tree before any
    // instance connections or interface bodies can refer to them.
    declaringHierarchicalOutputs = true;
    if (failed(predeclareModuleInstanceScope(scope, blockNamePrefix)))
      return failure();
    declaringHierarchicalOutputs = false;
